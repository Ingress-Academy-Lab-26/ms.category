...
class CategoryServiceTest extends Specification {
    ...
    def "getCategories() method should save categories to cache if cache is empty and return built category tree"() {
        given: "cache is empty initially"
        def dbCategories = [new CategoryEntity(id: 1L, name: "Category 1", baseId: null)]
        cacheServiceHandler.getCategoriesFromCache() >> null >> dbCategories

        and: "the repository returns categories from the database when saveCategoriesToCache is called"
        categoryRepository.findAll() >> dbCategories
        ...
        then: "saveCategoriesToCache is called to populate the cache"
        1 * cacheServiceHandler.saveCategoriesToCache()

        and: "categories are retrieved from the database, saved to cache, and returned as a tree"
        result == categoryTree
    }

    def "getCategories() method should return empty list if saveCategoriesToCache throws an exception"() {
        given: "cache is empty and saveCategoriesToCache throws an exception"
        cacheServiceHandler.getCategoriesFromCache() >> null
        cacheServiceHandler.saveCategoriesToCache() >> { throw new ExecutionException("Error", new Throwable()) }
